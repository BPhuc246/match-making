...
export default function MatchPage() {
  ...
  // 1. Initial fetch + subscribe to live round updates (no polling)
  useEffect(() => {
    if (!matchId) return;
  ...
  // 2. Reset local selection when a new round starts
  useEffect(() => {
    const latestRound = currentMatch?.rounds[currentMatch.rounds.length - 1];
    if (latestRound?.status === "PENDING" && !latestRound.myChoice) {
      setLocalChoice(null);
    }
  }, [currentMatch]);

  // 3. Clear the lobby's "matched" flag once we've actually landed here
  useEffect(() => {
    dispatch(clearMatchedMatchId());
  }, [dispatch]);

  // 4. Toast + redirect when match finishes
  useEffect(() => {
